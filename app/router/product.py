...
def list_products(db: Session = Depends(get_db), get_api_key: str = Depends(get_api_key)):
    return crud.get_products(db)

# Get specific product
@router.get("/{product_id}")
def get_product(product_id: int, db: Session = Depends(get_db), get_api_key: str = Depends(get_api_key)):
    return crud.get_product(db, product_id)

# Create Products
@router.post("/", response_model=BaseResponse[ProductOut])
def create_product(product: ProductCreate, db: Session = Depends(get_db), get_api_key: str = Depends(get_api_key)):
...
def update_product(product_id: int, product: ProductUpdate, db: Session = Depends(get_db), get_api_key: str = Depends(get_api_key)):
...
def update_patch_product(product_id: int, product: ProductPatch, db: Session = Depends(get_db), get_api_key: str = Depends(get_api_key)):
...
def remove_product(product_id: int, db: Session = Depends(get_db), get_api_key: str = Depends(get_api_key)):
